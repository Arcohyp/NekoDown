...
- ⏱️ **aria2 超时保护**：添加 `--timeout=60`、`--connect-timeout=30`，防止网络挂起导致下载无限卡住
- ➕ **下载中追加任务**：解析新链接后可直接追加到当前下载队列，无需等待全部完成
- 🔔 **卡住检测**：30 秒无进度更新自动提示"疑似卡住"
- 🚪 **关闭提示**：退出弹窗增加小字提示"退出将终止所有正在进行的下载"
- 🎨 **主题修复**：深浅色主题下按钮悬停颜色适配，文字始终清晰可读
- 🐛 **空 path 修复**：Cloudreve API 返回的文件缺少 path 时自动构建 fallback，解决"参数不能为空"错误
